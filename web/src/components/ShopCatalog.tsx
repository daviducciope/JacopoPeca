...
import type { AppCategory, AppProduct } from "@/lib/product-data";
...
export function ShopCatalog({ categories, products }: ShopCatalogProps) {
  return (
    <Suspense fallback={<ShopCatalogContent categories={categories} products={products} />}>
      <ShopCatalogWithSearch categories={categories} products={products} />
    </Suspense>
  );
}

function ShopCatalogWithSearch({ categories, products }: ShopCatalogProps) {
...
function ShopCatalogContent({
  ...
  products,
  selectedCategory,
}: ShopCatalogProps & { selectedCategory?: string }) {
      ...
      <section className="cement-bg cement-overlay relative">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-6 py-16 lg:px-10">
          ...
          <h1 className="mt-2 font-display text-6xl text-white md:text-8xl">
            {filteredProducts.length} pezzi unici
          </h1>
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <p className="text-sm font-semibold text-white/80">{formatPrice(product.priceCents)}</p>
                    {product.sizes.length > 0 && (
                      <p className="text-[10px] text-white/30">{product.sizes.length} taglie</p>
                    )}
                  </div>
                </div>
              </Link>
